...
def remove_k_digits(num: str, k: int) -> str:
    ...
    return {'The minimum number after removing \'k\' digits from the string \'num\'': ''.join(stack).lstrip('0') or '0'}
...
def get_payment_table_and_remaining_payment_list() -> tuple[dict[tuple, tuple], list[tuple]]:
    """
...
def get_list_of_not_used_payments_and_payment_table()  -> dict[str, Any]:
    ...
    'List of not used payments': 
        [{'id': id, 'date': str(date), 'month': month} for id,date,month in list_of_not_used_payments]
        }
